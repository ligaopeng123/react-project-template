...
class FormBrnSelectTag extends StatefulWidget {
  /// 展示的标签列表
  ...
  @override
  FormBrnSelectTagState createState() => FormBrnSelectTagState();
}

class FormBrnSelectTagState extends State<FormBrnSelectTag> {
  List<bool> _tagState = [];
  List<bool> _defaultTagState = [];

  @override
  void initState() {
    super.initState();
    _tagState = widget.tags.map((name) => false).toList();
    if (widget.initTagState != null && (widget.initTagState ?? []).length > 0) {
      for (int index = 0;
          index < min(widget.initTagState!.length, widget.tags.length);
          index++) {
        _tagState[index] = widget.initTagState![index];
        _defaultTagState = _tagState;
      }
    }
  }
  ...
  resetInitTagState() {
    setState(() {
      if (_defaultTagState.length > 0) {
        _tagState = _defaultTagState;
      } else {
  ...
  @override
  void didUpdateWidget(FormBrnSelectTag oldWidget) {
